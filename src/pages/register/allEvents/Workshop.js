...
const Workshop = (props) => {
    ...
    React.useEffect(() => {
        if (props.registeredEvent) {
            setFinalData(data.filter((item) => props.workShopsIds?.includes(item.id)));
        } else {
            setFinalData(data.filter((item) => !props.workShopsIds?.includes(item.id)));
            setFinalData((value) => [...value, ...data.filter((item) => props.workShopsIds?.includes(item.id))]);
        }
    }, [props.workShopsIds]);
            ...
            >
                {finalData.map((IData) => (
                    <EventCard
                        ...
                        isAlreadyRegistered={props.workShopsIds?.includes(IData.id)}
                    />
                ))}
            </div>
        </>
